...
setwd('/Users/ccruzr/Library/Mobile Documents/com~apple~CloudDocs/Cristian/Documents/Estudios/Postgrado/PhD/Projects/LPI_paper_NatureOfTimeSeries_Cristian-Gaelle')
route <- '02_outdata/'
...
species_data2 <- as.data.frame(scale(species_data))
...
species_data_subset <- species_data_clean[, c(300:370)]

# Save cleaned data to disk
dir.create(paste0(route, 'SimDat_real_Shape/'))
save(species_data_subset, file = paste0(getwd(), '/',  route, 'SimDat_real_Shape/simulations.RData'))

# Load and plot subset data
load(file = paste0(getwd(), '/',  route, 'SimDat_real_Shape/simulations.RData'))
...
ggsave(
 ggplot(species_data_final, aes(Year, Value)) +
    ...
    labs(x = "Year", y = "Count"),
 filename = "03_figures/_Density_Stable_popualtion_Sim_data_Full.png", 
 width = 180, height = 120, unit = "mm", dpi = 300
)

# Compute LPI using simulated data
lpi_result <- LPIMain(
  create_infile(species_data_final, index_vector = TRUE, name = 'Complete_DataSet', 
...
lpi_data <- read.csv('00_rawdata/LPD2022_public.csv')
...
lpi_combined_result <- LPIMain(
  create_infile(lpi_data_filtered_resampled, index_vector = TRUE, name = 'Complete_DataSet',
                start_col_name = "X1950", end_col_name = "X2019", CUT_OFF_YEAR = 1950),
 ...
 filename = "03_figures/_Density_Stable_popualtion_Sim_data_Real_Struct.png", 
 width = 180, height = 120, unit = "mm", dpi = 300
 )
...
lpi_window_result <- LPIMain(
  create_infile(species_data_window_resampled, index_vector = TRUE, name = 'Complete_DataSet',
                start_col_name = "X1950", end_col_name = "X2019", CUT_OFF_YEAR = 1950),
 ...
 filename = "03_figures/Windown_moved_Density_Stable_popualtion_Sim_data_Real_Struct.png", 
 width = 180, height = 120, unit = "mm", dpi = 300
 )
...
for (i in 1:length(lpi_trend_matrices)) {
  high_results[[i]] <- LPIMain(
    create_infile(lpi_trend_matrices[[i]], index_vector = TRUE, 
                  name = paste0(route, 'Simdata_high_population_outputs/', trend_names[i]),
                  start_col_name = "X1950", end_col_name = "X2019", CUT_OFF_YEAR = 1950),
...
Join_lpt_trends <- LPIMain(
    create_infile(join_trends, index_vector = TRUE, 
                  name = paste0(route, 'Simdata_high_population_outputs/join_trends'),
                  start_col_name = "X1950", end_col_name = "X2019", CUT_OFF_YEAR = 1950),
 ...
 filename = "03_figures/Join_thrends_Con_line_convex.png", 
 width = 180, height = 120, unit = "mm", dpi = 300
 )
