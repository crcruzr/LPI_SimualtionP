...
plot(as.numeric(species_data_final[sample(S, 1),]))

# Add structure real data
lpi_data_filteredNA[!maskNA] <- species_data_final[!maskNA]
table(maskNA)
table(is.na(lpi_data_filteredNA))
identical(is.na(lpi_data_filteredNA), is.na(lpi_data_filtered))
...
lpi_combined_resultNA <- LPIMain(
  create_infile(lpi_data_filteredNA, index_vector = TRUE, name = 'lpi_temp/complete/simulated/Complete_real_templateII_NAonly',
                start_col_name = "X1950", end_col_name = "X2019", CUT_OFF_YEAR = 1950),
  title = 'LPI Results Simulated Data - Real Dataset - only NA', REF_YEAR = 1950, PLOT_MAX = 2019, BOOT_STRAP_SIZE = 1000, VERBOSE = FALSE
)
colr <- c("#9467bd", "#c5b0d5")  # Purple + lighter purple
lpi_combined_resultNA$years <- years
p4 <- plot_lpi(data = lpi_combined_resultNA, colr = colr, label_name = "Simulation Simulation - Real Template - Only NAs")
p4

#Add structure real data
lpi_data_filtered0[!mask0] <- species_data_final[!mask0]
identical(is.na(lpi_data_filtered0), is.na(lpi_data_filtered))
lpi_data_filtered0 <- bino_id(lpi_data_filtered0, years, S)

# Compute LPI with the combined dataset
lpi_combined_result0 <- LPIMain(
  create_infile(lpi_data_filtered0, index_vector = TRUE, name = 'lpi_temp/complete/simulated/Complete_real_template_0sonly',
                start_col_name = "X1950", end_col_name = "X2019", CUT_OFF_YEAR = 1950),
  title = 'LPI Results Simulated Data - Real Dataset - Only 0s', REF_YEAR = 1950, PLOT_MAX = 2019, BOOT_STRAP_SIZE = 1000, VERBOSE = FALSE
)

lpi_combined_result0$years <- years
ggplot_lpi(lpi_combined_result0)

p5 <- plot_lpi(data = lpi_combined_result0, colr = colr, label_name = "Simulation  \n - Real Template \n - Only Zeros")
p5
